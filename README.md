...
![5f356fa45a29a0c2572d3b8dac02317](https://www.helloimg.com/i/2024/10/17/671131b3b3e16.png)
...
* 在跑通论文原始数据集之后，本组尝试使用自己的数据集进行测试。本组选用了2021年钱塘江浙江杭州桐庐流域的水质数据，采用多组变量预测单组变量的预测模式（利用水温，pH，溶解氧含量，高锰酸钾含量，氨氮含量，磷，氮，电导率，浊度这些数据预测水质）。经过调节模型主程序的参数、清洗数据集的脏数据之后，模型成功读取我们自己的数据集并跑通预测，由于提供的数据量较小且未进行模型调优，效果有待提高。
  ...
  ![自定义数据集](https://www.helloimg.com/i/2024/10/17/671131cec58ef.png)
...
* 本小组成员于本周阅读了部分马老师推荐的文献（来源于项目申报书），大致对水质预警的各种方案有了初步了解。现已将阅读文献并确定变量预测模式的任务布置给大三科研组以及科研一组。
* 本小组于本周研究了Informer模型的中文版论文（没读懂），同时参考多篇博客文章研究了源代码中main_informer.py主程序中的各个参数的意义，并写了一份文档供大一同学参考。

## 接下来任务

* 完成informer源码流程的理解（< 1 week）
* 性能调优（< 1 week）； 
...
* 本小组于本周将Informer整体的代码迁移到马老师提供的学校GPU服务器上，并在服务器上部署了Informer环境能成功跑起来，代码运行效率从本机的1小时变成了20秒，提高了我们调参的效率。

* 本小组在本周通过B站上的Informer原理网课，更加深入地了解了Transformer以及其改进版Informer的原理以及其正确的调用方式，并撰写了课堂笔记上传至gitee库

* 看完原理后，我们进行初步的调优。

  * 首先，我们经过查阅资料得知，水质应通过水的多项指标（如pH、氨氮、高锰酸钾等）进行if-else判断得出，所以我们考虑过后将我们的预测模式改为多变量预测多变量模式，预测各个指标未来的值的大小。
  * 然后，我们修改了输出预测图的方式，之前按照作者原论文的输出方式，导致效果不好，现在修改过后，发现预测数据与真实数据拟合程度大大加强。
  * 最后，在理解原理的基础上，我们去查看models.py中我们选用的Informer主类的各个变量以及语句的含义。在大模型的辅助下，我们大概理解了大部分的变量以及代码的含义并撰写了一份说明文档上传至gitee。同时，我们也进行了Informer主类模型的调优，我们尝试将`factor`（注意力机制中的因子）从默认的5调大，发现拟合程度比默认更高。也尝试将注意力类型从默认的`prob`（概率注意力机制）调整为`full`（全量注意力机制），但发现效果变差，遂改回默认。

  最终调优效果如下（截取一小个时间段）：

  ![image-20241104231900269](https://img.picui.cn/free/2024/11/04/6728e5b718135.png)

## 接下来任务

* 去除池化层，后续继续改代码（全局池化：会丢失部分数据），去除后复杂度提升，精度提高。
* 分两组，一组改loss函数（替换MAE，MSE），一组去除改池化函数。
* 位置编码(PositionalEmbedding)改进
* Transformer QK矩阵改进
...
* 本小组于上周去除了池化层，对比去除前后输出的loss数据，具体如下：

  有池化层：

  | MSE                                                          | MAE                                                          |
  | ------------------------------------------------------------ | ------------------------------------------------------------ |
  | 0.8256067633628845<br/>1.0405575037002563<br/>0.7775776386260986<br/>0.7516152262687683<br/>0.9095895290374756<br/>1.2214080095291138<br/>0.7659220099449158<br/>0.7621560096740723<br/>0.9279540777206421<br/>0.9652490615844727 <br /> <br /> avg: 0.894763583 | 0.6601687073707581<br/>0.7337061762809753<br/>0.6412497758865356<br/>0.6178722977638245<br/>0.7254443764686584<br/>0.821659505367279<br/>0.6606131792068481<br/>0.6454814076423645<br/>0.7380744218826294<br/>0.7460046410560608<br /> <br /> avg: 0.699027449 |
  ...
  | MSE                                                          | MAE                                                          |
  | ------------------------------------------------------------ | ------------------------------------------------------------ |
  | 0.8282049298286438<br/>0.8401686549186707<br />0.7945263981819153<br/>0.6834143996238708<br/>0.7878003716468811<br/>0.639975905418396<br/>0.9619460105895996<br/>0.9477255940437317<br/>0.671570897102356<br/>0.8085168600082397<br /> <br /> avg: 0.796385002 | 0.6884005069732666<br/>0.6950008869171143<br/>0.6447485089302063<br/>0.6050081253051758<br/>0.6624388098716736<br/>0.5976359248161316<br/>0.758580207824707<br/>0.7753211855888367<br/>0.6019493937492371<br/>0.6645607948303223 <br /> <br /> avg: 0.669364434 |

  

* 本小组于本周更新了新的loss函数——huber_loss函数，本损失函数介绍如下：
...
* 本小组现将delta值调为1，以1作为参考点判断模型的预测能力，但由于遇到未知问题，loss值来到了五位数，等待查明原因
